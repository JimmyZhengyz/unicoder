...
def process_data_on_gpu(device_index, data_chunk, output_file_path):
    ...
    for data in tqdm(data_chunk, desc=f"Processing on GPU {device_index}"):
        ...
        if data['type'] in ["ISVGEN", "ISVGUN_usage", "ISVGUN_category", "ISVGUN_description", "ISVGUN_color"]:
            image_path = os.path.join(image_dir, data.get('image_path', ""))
...
def main():
    global num_gpus, devices, model_path, image_dir

    parser = argparse.ArgumentParser()
    parser.add_argument("--data_file", required=True)
    parser.add_argument("--image_dir", required=True)
    parser.add_argument("--output_dir", default="results/unisvg_infer")
    parser.add_argument("--model_path", default="Qwen/Qwen3-8B-Instruct")
    parser.add_argument("--num_gpus", type=int, default=torch.cuda.device_count() or 1)
